...
- [x] Successful live compaction renders exactly one enriched generated summary first, followed by kept and post-compaction transcript entries in canonical order; compacted-away entries are absent (`interactive-mode-compaction.test.ts`: “renders one enriched compaction summary before kept messages after compaction”).
- [x] `/tree` user-only mode is compaction-aware on the active branch, showing the generated summary first followed by retained and later user prompts in canonical context order, while default and other filters retain the full persisted tree and pre-compaction tree behavior is unchanged (`tree-selector.test.ts`: “shows the generated compaction summary before retained active-context prompts”).
...
- `packages/coding-agent/test/interactive-mode-compaction.test.ts` and `interactive-mode-idle-notification.test.ts` — compaction/retry status labels, prompt-activity lifecycle, and one-summary transcript rendering after compaction.
...
- `packages/coding-agent/test/tree-selector.test.ts` — compaction-aware user-only tree ordering and preservation of the full tree in other filters.
