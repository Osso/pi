...
When launching a runtime runner, the backend read-only mounts its resolved executable, absolute shebang interpreter, Python virtual-environment metadata/site-packages, external base-runtime paths, absolute runner arguments, and `PYTHONPATH` entries when they are outside the workspace and not already available from system mounts. A symlinked runner command is resolved to its canonical launcher target before mounting and invocation; script launchers remain the executed command rather than being rewritten to their interpreter. This permits external runtimes to start without broad host mounts.
