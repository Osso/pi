...
optional labels, so logs and diffs remain direct file references rather than registry records. The direct `wait_agents({})`
...
The persistence slice writes per-agent and per-mailbox rows:

- Agent and mailbox rows persist in `control.sqlite`, keyed by session path.
- `MultiAgentStore.fromSessionManager()` rehydrates the session's agent and mailbox rows.
- Runtime mailbox transport stores only a required `storeRef`; payload bodies and `fileRefs` resolve
