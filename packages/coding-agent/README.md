...
Then just talk to pi. By default, pi gives the model file, command, and structural code tools including `read`, `write`, `edit`, and `bash`; `outline`, `symbol`, and `references` are included when the local `code-index` CLI is available on `PATH`. The structural code tools use `code-index` to inspect unfamiliar files before reading full contents, jump to known symbols, and find symbol references. Pi does not install missing tools at runtime. First-party binary-backed extensions load only when their executables are available at startup; restart Pi after installing or removing one. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [pi packages](#pi-packages).
...
For each built-in provider, pi maintains a list of tool-capable models, updated with every release. At startup, pi also adds new OpenRouter models from a catalog cached for seven days at `$XDG_CACHE_HOME/pi/models/openrouter.json` (default `~/.cache/pi/models/openrouter.json`). Run `pi --refresh-models` to fetch and persist a new catalog; it retries only transient network or HTTP failures, honors `Retry-After`, and exits nonzero with the cause if fetching or writing the cache fails. A failed refresh preserves the existing cache. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
- **Enter** sends a *steering* message: active model thinking is interrupted immediately, while active tool execution finishes before delivery
...
Pi searches the global agent directory (`~/.pi/agent/` by default) and every directory from the filesystem root through cwd for context files. It scans all those locations for AGENTS-family files first. If any AGENTS-family file loads successfully anywhere, Pi does not access CLAUDE-family paths anywhere; only when none load across the hierarchy does it load CLAUDE-family files. In each cwd ancestor, it then loads `docs/local/memory.md` when present, after that directory's selected instruction-file sequence. `docs/local/memory.md` is project context only: Pi does not load it from the global agent directory.
...
Place in `~/.pi/agent/skills/`, `~/.agents/skills/`, `.pi/skills/`, `.agents/skills/` (from `cwd` up through parent directories), or repo-local `.codex/skills/` and `.claude/skills/`, or a [pi package](#pi-packages) to share with others. See [docs/skills.md](docs/skills.md).
...
| `--refresh-models` | Fetch and persist the OpenRouter model catalog cache, then exit; reports failures and exits nonzero without replacing an existing cache |
