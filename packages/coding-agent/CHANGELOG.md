...
- Added a durable multi-agent recovery-leader lease with live-owner rejection, expiry takeover, monotonic fencing, exact renewal, and compare-release so competing sweepers cannot reconcile concurrently. Expired runtime recovery can now atomically verify that leader, take the next dispatch fencing epoch, and commit `failed/lost_runtime` with one terminal event/outbox.
