...
- Fixed durable foreground Pyrun wrapper failures leaving child agents indefinitely on `Waiting for tool: pyrun_eval`; pre-detach `.runner-error` sidecars now promptly terminalize the foreground call after canonical success/error records take precedence, surface nonempty sidecar text, report empty sidecars explicitly, and terminate the foreground process tree.
