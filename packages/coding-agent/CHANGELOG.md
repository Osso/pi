...
- Fixed active spawned-agent ghosts surviving in historical non-current supervisor stores: main listeners now freshly assert their exact session path, path relocation moves the assertion transactionally with the store, and listener startup plus session-directory synchronization abort stores with explicitly ended (`pid: NULL`) health or duplicate paths differing from a trusted live assertion. Pathless and legacy timestamp-only heartbeats invalidate assertion trust and fail safe until re-registration; missing-health, attached, queued, terminal, current live, and stale process-backed timeout records remain preserved.
