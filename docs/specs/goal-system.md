...
- [x] `/goal <objective>` establishes an active objective for the current working directory and persists it to `.pi/goal.json`.
...
- [x] Corrupt or malformed `.pi/goal.json` is handled as "no active goal" without crashing the command or turn hook.
...
- Uses project-local `.pi/goal.json` for transparent, per-branch state.
