...
- [x] `hideToolOutput: true` in settings.json (or "Hide tool output" in `/settings`, applied live to existing cards) hides tool result text and images in the transcript for built-in, extension, and definition-less tools, regardless of Ctrl+O expansion; call lines and elapsed timers stay visible and the model still receives full output (`tool-execution-component.test.ts`: "hideOutput keeps the call and drops result output until disabled", "hideOutput drops result output for tools without a definition"; `settings-manager.test.ts`: "hideToolOutput").
