...
| CLI session lookup (`--session <id>`) | [`session-cli-lookup.md`](session-cli-lookup.md) | **NATIVE** | Resolves current-project and global session IDs; same-cwd global matches open directly. |
...
| First-party `ask_secret` tool | [`ask-secret-tool.md`](ask-secret-tool.md) | **BUILD** (done) | Interactive-TUI-only browser credential provisioning and single-value secret-file persistence without returning secret values. |
...
| OpenRouter model catalog cache | [`model-catalog-cache.md`](model-catalog-cache.md) | **BUILD** | Fresh XDG cache augments bundled metadata during normal startup; `--refresh-models` is the only network refresh. |
...
| Session archive state | [`session-archive.md`](session-archive.md) | **BUILD** (done) | Control-DB archive state, Archived picker scope, Ctrl+A selected-session archive, age-based archive CLI, first-party `/archive` current-session archive-and-quit, and `/delete` current-session deletion with child agent sessions. |
| Session autonaming | [`session-autoname.md`](session-autoname.md) | **BUILD** (done) | Default first-party extension that asynchronously titles persisted unnamed main sessions from the first real-user message in interactive TUI and RPC modes. |
...
| Speculative background compaction cache | [`speculative-background-compaction-cache.md`](speculative-background-compaction-cache.md) | **BUILD** | Starts cache-only compaction at 70% context and commits it through normal safe-point compaction. |
...
| OpenAI Codex Responses transport | [`openai-codex-transport.md`](openai-codex-transport.md) | **NATIVE** | Explicit SSE or WebSocket selection; WebSocket-selected requests never fall back to SSE. |
| Codex fast mode (`/fast`) | [`codex-fast-mode.md`](codex-fast-mode.md) | **BUILD** (done) | Per-session priority/ultrafast processing toggle for OpenAI Codex providers, with an optional merged settings default used only when the session has no valid persisted fast-mode entry. |
