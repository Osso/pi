...
goal running and queues the generic active-goal reminder when the agent can
determine its own next step; it queues a specific corrective instruction only
for an evidence-backed omission, lost or narrowed scope, contradiction, repeated
or circular work, or missing completion proof. `complete` marks the goal complete
...
Goal reviews preserve only requirements grounded in explicit user instructions or applicable persistent project contracts. The goal extension supplies the latest genuine same-session input as `userRequest`, even before a goal exists; extension messages cannot replace it. Ordered conversation events remain separate, and assistant end-turn reports are not user instructions. Prior objectives, plans, summaries, diagnostic task IDs, and Supervisor/KB repetition cannot establish requirement provenance. Reviews remove unsupported constraints while retaining grounded unfinished scope and checking actual completion evidence.
