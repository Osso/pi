...
Every automatic Supervisor wait calculates one absolute fifteen-minute `reviewAt` deadline and persists that same value in its durable `supervisor-status` entry, including active-agent `wait_agent` waits. Interactive rendering derives `Next review in M:SS` from the deadline on each paint and switches to `Review due…` at expiry. A separate deadline-aligned refresher only requests UI redraws; it never appends entries, invokes Supervisor review, or changes the fifteen-minute schedule.
...
corrective instruction; `wait` appends durable status and schedules agent wake or fifteen-minute
