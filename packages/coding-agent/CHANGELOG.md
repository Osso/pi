...
- Renamed the child mailbox tool `contact_supervisor` to `contact_parent`; it is direct-parent-only, derives the exact caller runtime identity from session context instead of accepting an `agentId` argument, rejects parentless runtimes, validates persisted `parent_request` targets against the current direct parent, cannot target the resident Supervisor, and has no `contact_supervisor` compatibility alias.
- Renamed the model-facing `cancel_agent` tool to `close_agent`; it retains `agentId` and optional `reason`, terminal coordinator cancellation semantics, and has no `cancel_agent` compatibility alias.
...
- Changed `/archive`, picker, and bulk-archived session storage to compress transcripts as `.jsonl.zst`; resuming restores plain `.jsonl` storage and active metadata. Added `pi sessions compress-archived [--dry-run]` to migrate existing metadata-archived plain JSONL files while excluding live resident Supervisor and Architect transcripts and reporting migration results.
...
- Increased the automatic Supervisor goal-review wait interval from five to fifteen minutes.
...
- Changed automatic non-generic Supervisor continuation instructions to pass through a stateless veto gate that preserves only `ACCEPT` results; tactical-direction refusals, invalid output, and evaluation failures fall back to the generic goal reminder with hidden resident policy feedback.
...
- Added a live `Next review in M:SS` countdown to every automatic Supervisor wait status, including active-agent `wait_agent` waits, with one persisted absolute fifteen-minute deadline, early agent-wake review, deadline cancellation of `wait_agent`, first-path-wins cancellation, `Review due…` expiry state, restored rendering, and no duplicate reviews or transcript entries.
...
- Added a distinct Supervisor `wait` goal decision that emits one durable status with one absolute fifteen-minute deadline, runs cancellable background `wait_agent` when agents are active, reviews early on agent completion, cancels `wait_agent` and reviews at the deadline, and lets the first path win without duplicate work; idle-review errors use the same self-waking path, transient pending-message suppression retries before review or preserves an already reviewed decision, agent wake evidence survives pre-review retries, asynchronous reviews recheck cancellation generation and goal identity, `wait_agent` failures retain the original deadline for timed review, and input, goal changes, new turns, or shutdown cancel stale work.
- Added proof-backed `completionReport` handling to `manage_goal complete`: nonblank free-form Markdown is required locally, passed verbatim through Supervisor wait/re-review, persisted as `completionReason` only on completion, and retained with Supervisor rejection reasons in durable status without automatic evidence inference.
- Added ordered goal-review conversation evidence: exact interactive/RPC user text and successful `end_turn` reasons persist for running or paused goals, replace `terminalTurn` in idle reviews, extend completion reviews, survive failed or canceled reviews, and clear when goals are completed, replaced, or cleared; Supervisor instructions are not re-sent.
...
- Added `background_compaction_start`/`background_compaction_end` extension events with guaranteed terminal pairing, and a default-footer `background-compaction` status showing `compacting context` during active generation.
- Added a disposable real-process headless Pi test fixture built on RPC mode, with isolated state, typed raw commands, private faux-provider response control, persisted agent/mailbox waiters, shared-session creation/resume controls, a listener-registration barrier before `session_start`, and automatic cleanup; `RpcClient.send()`, `RpcCommandBody`, and `RpcClientOptions.nodeArgs` are now public for raw commands and preload instrumentation.
...
- Added pre-persistence durable tool execution metadata for detachable tools: Pyrun stores its application-owned `agentId` on the assistant tool call, reuses it during interrupted replay, and opens only `detached-jobs/<session>/<agentId>/launch.json` instead of scanning historical job manifests. Provider tool-call IDs remain separate for immediate call/result correlation.
...
- Added a shared detached-job lifecycle controller adapter covering identity allocation, diagnostic artifacts, coordinator process ownership, projection publication, and direct in-memory finalization for Bash/Pyrun callers.
...
- Added an exact detached-job `finalize` repository transaction that validates in-memory identity, outcome, output metadata, and exact runner process identity, restricts settlement to running/cancelling jobs, and atomically commits terminal state and its delivery outbox with idempotent retry.
- Added a shared detached-job runner artifact contract with identity-bound directories and direct durable output paths; output files are retained as diagnostic artifacts, not lifecycle evidence.
- Added persistent session archiving with an Archived resume-picker scope, Ctrl+A selected-session archiving, `pi sessions archive --older-than <days>`, first-party `/archive` archiving of the current persisted session, and `/unarchive` clearing archive state only for the current persisted session without resuming, replacing, or switching transcripts.
...
- Added `/tree` compaction-summary editing: plaintext summaries load directly, while OpenAI-native entries are materialized through one current provider/API-matching model call with no fallback; materialization is non-persistent, Escape/failure leaves the entry unchanged, and saving replaces native context with plaintext while preserving entry identity, leaf, and token/duration metadata.
...
- Added `Ctrl+B` backgrounding for running bash tool calls, moving detached commands into `/jobs` with direct log file references and `close_agent` support.
...
- Fixed a cancelled turn's post-run handler racing manual-compaction resumption: while the resumed turn streams, the original handler defers without consuming queued follow-up or starting a duplicate continuation.
...
- Fixed subagents waiting indefinitely after their turn ended while directly owned detached Bash/Pyrun jobs remained active; terminal paths now cancel only active direct children with `parentId` set to the subagent, `agentType: "background"`, `worker.adapter: "runtime"`, and `detached: true`, persist that silent policy across supervisor restart, retain terminal outbox evidence, and suppress only terminal-notification projections, runtime terminal transport, and `detached_tool_call_completion` entries. Main-session jobs and direct `close_agent` cancellation retain normal notifications.
- Fixed durable Pyrun cancellation killing only the nested runner while leaving its child processes alive; Linux cancellation now terminates inherited process-group members without detaching the nested runner from the durable wrapper.
...
- Fixed PyrunRunnerClient parsing of large newline-free JSONL stdout chunks to avoid quadratic buffering, preventing restored-child large-output timeouts and OOM while preserving ordered protocol handling.
...
- Fixed lifecycle mutations reserving SQLite's writer slot during ownership/lifecycle validation and detached-cancellation payload preparation; validation and payload serialization now run before the transaction, while exact snapshot revalidation keeps lifecycle and cancellation persistence atomic.
- Fixed steering mutations reserving SQLite's writer slot during agent, ownership, transition, listener, and mailbox-payload preparation; validation and agent serialization now run read-only, while a validated generated-ID mailbox template is completed inside the atomic transaction only after counter allocation, so rollback preserves numbering and agent/mailbox persistence.
- Fixed steering delivery reserving SQLite's writer slot during agent, ownership, transition, and canonical message validation; exact agent and message snapshot revalidation now keeps delivery's coupled row updates atomic, with updated payload serialization completed before lock acquisition.
- Fixed terminal mutations reserving SQLite's writer slot during ownership, replay, transition, and descendant validation; read-only preflight now precedes the transaction, including updated-agent serialization, while commit-time recursive descendant CAS preserves exact ownership and atomically persists the terminal agent and outbox row.
- Fixed detached-job finalization reserving SQLite's writer slot during ownership/path lookup, replay, lifecycle, and descendant validation; read-only preflight now resolves one exact owner path and prepares detached transport JSON/routing, while commit-time uniqueness and recursive descendant CAS atomically persist the terminal agent, outbox row, and direct insert of the prepared transport payload.
...
- Fixed durable foreground Pyrun wrapper failures leaving child agents indefinitely on `Waiting for tool: pyrun_eval`; pre-detach `.runner-error` sidecars now settle only after already-present artifact bytes are drained so canonical success/error records take precedence, surface nonempty sidecar text, report empty sidecars explicitly, and terminate only the exact still-live foreground runner identity.
...
- Fixed runtime-mailbox claim, delivery, and dead-claim recovery payload preparation from reserving SQLite's writer slot; transition serialization now runs before each short transaction, which retains only authority revalidation and exact-payload compare-and-swap under the lock.
...
- Changed browser agent runtimes to add the standard `read` tool alongside `browser-cli` and lifecycle tools even when a configured browser tool allowlist omits it; browser reads canonicalize every target and permit only paths whose realpath is under `~/AgentConfig/**`, allowing direct paths and in-tree symlink aliases while rejecting ordinary outside paths and symlink escapes. Spawned and attached browser sessions share the restriction; main and non-browser reads remain unchanged.
...
- Changed Supervisor request clients on Linux and macOS to connect-or-spawn one detached resident automatically, serialize concurrent startup, reuse compatible externally managed services, and replace only incompatible Pi-managed residents; systemd remains optional.
...
- Resident Supervisor now awaits reload of its caller-provided resource loader before `createAgentSession`; with `noExtensions: true`, the explicit extension allowlist contains only the Supervisor mutation gate. Generic active-session compaction remains the default, and persistent Codex transcript overflow recovers through normal AgentSession compact-and-retry.
...
- `wait_agent({})` now returns a transient `wake_up` when steering is accepted for a snapshotted active agent during a live wait; persisted coordination remains detected by the three-second polling loop, the wake is not persisted or replayed, `wake_up` does not use SIGUSR2 or mailbox transport, the steering mailbox row remains persisted, and a terminal row observed at wake takes precedence.
- Main-session shared-channel delivery now wakes an active `wait_agent({})` immediately after the prompt is accepted and the recipient cursor advances; the wake returns the exact formatted coordination prompt, while terminal agent notifications take precedence.
...
- User rules now support runtime-scoped `rules/main/`, `rules/child/`, and resident-Architect-only `rules/architect/` directories; the public `createAgentSession({ rulesScope })` override takes precedence over runtime-role defaults, ordinary observer runtimes receive only shared rules, and the resident Architect loads shared plus `rules/architect/` rules. Architect findings are advisory hypotheses only: it does not issue commands, execution gates, stop conditions, operational sequencing, mandatory next steps, or reinterpret goal completion.
...
- Terminal completion and failure notifications now come from the delivery outbox after the agent row commits. Runtime transport uses canonical session-bound mailbox rows shared by direct tools and the Pyrun handler instead of duplicate per-dispatch and session-global mirrors. Outbox claims now expire and retry the same deduplicated notification, poison exhausted attempts, require claim-scoped acknowledgement, and clean retained delivered/poisoned rows after seven days. `wait_agent` keeps agent rows as terminal truth, supports simultaneous and late waiters, and on coordination wake returns and consumes all currently pending deliverable runtime-mailbox and shared-channel inputs with preserved sender/body formatting; mailbox rows become delivered and the shared-channel cursor advances so each distinct message is visible once.
...
- Routed production `spawn_agent` child creation and runtime startup through `LifecycleCoordinator`: child construction happens first, successful construction persists `running` revision 1 with exact process ownership, and construction interruption or failure persists `failed` revision 1 with error details. Sessionless orchestration fails closed.
...
- Changed the opt-in Codex remote-compaction extension to use the active provider, account, and model while retaining native history across generation-model switches on that provider.
...
- Changed `wait_agent` from a per-agent `{ agentId }` call to a zero-argument wait-any tool. It immediately consumes every pending terminal notification already waiting or waits for any agent active at invocation to reach terminal state; the direct tool returns the drained completion/status set while Pyrun `pi.agents.wait()` returns `null`.
- Split agent notification waiting from notification consumption so internal callers can wait for terminal or coordination input without consuming completion delivery; `wait_agent({})` preserves its existing behavior by composing both operations.
...
- Fixed attached-agent recovery passing a deleted persisted child cwd into runtime creation; recovery now reports the agent and missing path before runtime creation.
...
- Fixed Architect claim renewal issuing one write per request ID and allowing duplicate IDs to repeat the renewal; renewal now deduplicates IDs, uses one set-based update, and validates all requested claims atomically while accepting completed rows.
- Fixed idle Supervisor request recovery acquiring the shared `control.sqlite` writer lock; recovery now probes for expired or claimed work read-only, while expired-request completion and claimed-request requeue remain one immediate transaction.
- Fixed generic agent bootstrap acquiring the shared `control.sqlite` writer lock before rejecting process-owned rows; bootstrap now rejects those rows from read-only state and uses one ownership-guarded upsert for unowned rows.
- Fixed runtime-owned child creation acquiring the shared `control.sqlite` writer lock for pure payload, ID, lifecycle, parent-ID, and serialization validation; parent lifecycle and uniqueness now preflight read-only, while commit-time exact-parent CAS preserves atomic child-plus-ownership persistence.
- Fixed attached-agent creation acquiring the shared `control.sqlite` writer lock for pure payload, ID, origin, lifecycle, revision, and serialization validation; parent lifecycle, permission, and uniqueness now preflight read-only, while exact-parent CAS preserves atomic insertion.
- Fixed failed-child creation acquiring the shared `control.sqlite` writer lock for pure payload, lifecycle, revision, parent-ID, and serialization validation; commit-time exact-parent CAS preserves atomic failed-child-plus-terminal-outbox persistence.
- Fixed concurrent multi-agent agent and message ID allocation using a read/increment/upsert sequence that could race; allocation now uses one atomic incrementing upsert with `RETURNING`, preserving unique contiguous values under concurrent workers.
...
- Fixed multi-agent activity, transcript, and slot metadata updates reserving the shared `control.sqlite` writer lock for validation; updates now validate read-only, retry bounded exact-payload compare-and-swap writes, and guard activity writes by exact persisted ownership in the update predicate.
- Fixed agent detachment validation waiting behind an unrelated `control.sqlite` writer; detachment now rejects stale or foreign exact ownership without side effects, while valid marks commit the detached state and revision atomically and repeated marks remain idempotent.
...
- Fixed detached-runtime reconciliation holding the shared `control.sqlite` writer lock while scanning candidates and checking runner liveness; candidate preparation now runs read-only and each confirmed dead owner is revalidated and committed in its own short transaction.
- Fixed direct dead-runtime recovery holding the shared `control.sqlite` writer lock during supervisor, agent, owner, descendant, and process-liveness validation; recovery now preflights those checks and detached transport JSON/routing read-only, revalidates persisted supervisor/listener authority, exact owner and agent snapshots, and recursive descendant absence with commit-time CAS, then atomically persists ownership release, terminal state, the outbox row, and the prepared transport insert under the short lock.
- Fixed attached runtime ownership acquisition holding the shared `control.sqlite` writer lock during supervisor, agent, ownership, and process-liveness checks; existing exact ownership now returns read-only, while takeover revalidates snapshots before atomically updating ownership and agent revision.
...
- Fixed dead runtime-mailbox claim recovery holding the shared `control.sqlite` writer lock while scanning claims and checking claimant liveness; recovery now prepares candidates outside the lock and revalidates each candidate in a short per-claim commit.
- Fixed runtime-mailbox claim authority scans holding the shared `control.sqlite` writer lock; claim candidates now prepare outside the lock and revalidate exact authority and payload state in short per-candidate commits.
- Fixed unchanged runtime-mailbox routing acquiring the shared SQLite writer lock; unchanged routes now return from read-only state, while changed routes use bounded payload compare-and-swap retries before notifying the listener.
- Fixed multi-agent mailbox upserts holding the shared `control.sqlite` writer lock during parent-target, collision, and merge preparation; unchanged rows now return read-only, while changed or inserted rows revalidate exact parent and mailbox snapshots before one atomic upsert with bounded conflict retries.
- Fixed duplicate or conflicting stored runtime-mailbox enqueues acquiring the shared SQLite writer lock; existing rows now validate from read-only state, while absent rows use one conflict-safe insert and validate a concurrent winner without rewriting it.
...
- Fixed claimed and direct runtime-mailbox delivery updates taking an unnecessary immediate transaction; status transitions now use atomic compare-and-swap writes guarded by the observed payload and claimant identity.
...
- Fixed goal-managed `wait_agent` wakeups to preserve visible coordination content, preventing shared-channel instructions from being discarded before Supervisor re-review.
...
- Fixed `wait_agent({})` to return and deliver every terminal notification already waiting instead of requiring one call per notification.
...
- Fixed opening, continuing, or forking compacted sessions looping or exhausting memory when a reverse-read chunk began exactly at a newline; current-version loading now reverse-scans the active parent chain through `firstKeptEntryId`, retains only the active slice in memory, preserves the untrimmed transcript and immutable initial-cwd header on disk, restores current cwd/model/thinking from control SQLite, uses configured defaults without backfill when model/thinking metadata is absent, and rejects corrupt active JSONL while ignoring one incomplete trailing line.
...
- Fixed debug REPL evaluations completing after client disconnect without writing to the closed socket or crashing Pi; settlement and audit still complete.
...
- Fixed main and child sessions remaining in a model thinking phase indefinitely; each thinking phase now aborts after 15 minutes, child timeout failures terminalize as failed, tool execution remains uncapped, and post-tool or steered model turns receive fresh deadlines.
...
- Fixed OpenAI remote compaction retaining hundreds of provider-generated message rows with identical role, status, and content but different response-item IDs; native replacement history now keeps only the latest duplicate message while leaving other item types unchanged.
...
- Fixed detached `running` and `cancelling` jobs remaining wedged after their exact runner died: startup globally reconciles rows whose dead runner identity matches the persisted worker handle, has no active descendant or replacement owner, and has no terminal outbox, regardless of parent-session liveness, without replaying artifacts, reparenting, mutating the owner agent, or duplicate terminalization.
- Fixed session-start recovery races: listener registration now completes before `session_start` (and registration failure prevents the event), a paused same-session recovery listener blocks foreign sweeps, and concurrent foreign peers serialize to one terminal commit.
- Fixed headless Pi fixture disposal skipping primary client, runner, provider, or temp cleanup when shared-session disposal failed; shared and primary cleanup now both run and failures are aggregated.
- Fixed session-load recovery never settling a wedged `cancelling` detached job: dead-owner recovery no longer requires the recorded owner session ID to match the current incarnation (every resume registers a fresh session ID, making that check unsatisfiable). Authorization rests on exact persisted runner ownership plus proof the owner process is dead, and a recovered `cancelling` agent settles as `aborted/lost_runtime`, honoring the recorded cancellation intent.
...
- Fixed agents-core session startup dereferencing a missing session manager while restoring compacted parent-agent journal records; contexts without session persistence now skip that restoration path.
...
- Fixed lifecycle protocol migration reserving SQLite's writer before process-liveness checks: quiescence validation now runs read-only first, and the atomic migration fence revalidates the exact persisted listener, health, owner-table, and process-identity snapshot before converting legacy rows.
...
- Fixed `close_agent` rejecting persisted detached Pyrun evaluations; persisted Bash and Pyrun jobs now derive their exact runner-control identity from the runtime worker and durable output label, then use the same runtime-mailbox cancellation transaction.
- Fixed detached Bash/Pyrun cancellation racing payload exit: when cancellation commits before direct exact-owner finalization, the repository treats the runner's exit acknowledgement as required and atomically records `aborted`.
- Fixed dead detached runners being reassigned to the supervisor before recovery; if a runner dies before terminal commit, exact-owner reconciliation records `failed/lost_runtime` from `running` or `aborted/lost_runtime` from `cancelling` without replaying diagnostic output.
...
- Removed Node-only SQLite lifecycle-authorization UDFs and triggers that crashed standalone Bun startup; lifecycle authority remains enforced by coordinator/repository construction, exact process-owner transaction predicates, and schema-version startup checks, with production source-scan checks retained as a static guard.
...
- Fixed selected-child views to identify the agent directly, report the selected agent or main thread after `/agents` selection, show the selected child's model and thinking level in the footer, and hide the main working loader without clearing unrelated status while a child transcript is displayed.
- Fixed active spawned-agent ghosts surviving in historical non-current supervisor stores: main listeners now freshly assert their exact session path and exact `(pid, startTimeTicks)` process identity, path relocation moves the assertion transactionally with the store, startup reconciliation retires non-Pi listener ownership before trusting asserted paths, and listener startup plus session-directory synchronization abort stores with explicitly ended (`pid: NULL`) health, replaced runtimes even after same-PID reuse, or duplicate paths differing from a trusted live assertion. Pathless and legacy timestamp-only heartbeats invalidate assertion trust and fail safe until re-registration; missing-health, attached, terminal, current live, and stale process-backed timeout records remain preserved.
...
- Fixed multi-agent restart recovery to mark detached children `failed/lost_runtime` from `running` or `aborted/lost_runtime` from `cancelling` when their runner dies before terminal commit, restricted orchestration and supervisor-wide reconciliation to main runtimes, and prevented in-process children from replacing the same-PID supervisor listener.
...
- Fixed the required `end_turn` loop intercepting `"length"`-truncated responses before threshold compaction could compact and resume them.
...
- Fixed agent inspection tool content to identify listed agents by ID, name, type, status, and lifecycle, and to show agent viewer status plus terminal summaries or errors.
...
- Fixed pending lifecycle notifications being stranded after startup or a transient mailbox enqueue failure: startup and `wait_agent({})` now retry mirroring before notification consumption.
...
- Fixed prompt history persistence to use the shared control SQLite database so concurrent Pi sessions append entries instead of overwriting each other; legacy rows are now trimmed and deduplicated read-only, blank migration input takes no writer lock, and nonblank migration uses one conditional bulk insert.
