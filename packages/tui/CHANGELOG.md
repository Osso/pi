...
- Added a non-fallback render-region attempt for callers that must skip unsafe partial updates without scheduling a normal root render.
- Added bounded flow render regions so a registered flow component can append, update, or grow without rerendering earlier flow components. Cached downstream flow and bottom rows, terminal scrollback, registered regions, fixed cells, and the hardware cursor remain coherent; unsafe or shrinking updates fall back to normal root rendering (`render-region.test.ts`: "appends and routes a new descendant from an empty flow tail", "grows a flow tail without rendering surrounding components", "grows a visible flow tail while preserving terminal scrollback", "preserves a bottom hardware cursor while a flow tail grows", "shifts a fixed bottom cell after flow-tail growth", "shifts a registered bottom region after flow-tail growth", and "falls back to a normal root render when a flow tail shrinks").
...
- Reset an upward-moving viewport when content shrinks after a tall temporary editor or selector closes, restoring bottom-anchored editor and footer rows (`root-compositor.test.ts`: "restores bottom anchoring after a tall editor replacement closes with Escape").
