...
- [x] `spawn_agent` accepts an optional `context` value of `"fresh"` or `"inherit"`. Omitted context
      ...
      Pyrun, detached Pyrun, and sequential-sibling execution. `"inherit"` rejects an unpersisted parent
...
- `LifecycleCoordinator` is the sole control-plane and runtime-command authority. Spawn, attach,
  ...
  Detached Bash and Pyrun runners directly call the narrowly scoped repository terminal-finalize transaction using their in-memory identity, outcome, and output metadata. Full-access detached Pyrun inherits the launching Pi environment; explicit `inheritEnv: false` remains isolated to `runnerOptions.env`, and inherited values are never persisted in launch manifests.
...
- [x] `/model` and `/effort` target the currently viewed live session: the main session when main is viewed,
      or the viewed live subagent session when a subagent is selected.
- [x] Selecting a live subagent for `/model` or `/effort` leaves the main session unchanged.
