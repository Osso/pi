...
function createTranscriptSwitchFixture(options: {
	...
	const fakeThis = {
		...
		settingsManager: { getHideToolOutput: () => false, getImageWidthCells: () => 80, getShowImages: () => false },
...
describe("InteractiveMode key handlers", () => {
	...
	test("escape while Supervisor review is active during streaming cancels the review first", () => {
		const actions = new Map<string, () => void>();
		const cancelSupervisorReview = vi.fn(() => true);
		const fakeThis = {
			defaultEditor: {
