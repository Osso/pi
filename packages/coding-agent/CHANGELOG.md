...
- Fixed direct dead-runtime recovery holding the shared `control.sqlite` writer lock during supervisor, agent, owner, descendant, and process-liveness validation; recovery now preflights those checks read-only, revalidates persisted supervisor and agent ownership in the mutation transaction, and commits only the ownership release and terminal rows under the short lock.
