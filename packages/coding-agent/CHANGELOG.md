...
- Fixed Pyrun streamed console progress retaining unbounded newline-free output by capping recent text to one MiB while preserving its tail.
...
- Fixed historical detached cancellations remaining wedged after their supervisor died: startup now refreshes current runtime bindings and globally reconciles only rows whose recorded owner session is sticky-dead, has no live listener for that owner session ID, has a dead runner identity matching the persisted worker handle, and has no terminal outbox, through the canonical lost-runtime path, without proving an owner-session/path match, reparenting, or duplicate terminalization.
...
- Fixed dead detached runners being reassigned to the supervisor before recovery; if a runner dies before terminal commit, the owning supervisor resolves the exact dead owner as `failed/lost_runtime` without replaying diagnostic output.
...
- Fixed multi-agent restart recovery to mark detached children `failed/lost_runtime` when their runner dies before terminal commit, restricted orchestration and supervisor-wide reconciliation to main runtimes, and prevented in-process children from replacing the same-PID supervisor listener.
