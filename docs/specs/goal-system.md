...
- [x] Autonomous continuation has no numeric turn cap; a Supervisor `continue` decision submits actionable instructions as the next follow-up, `complete` closes the goal, and `pause` or `error` visibly stops automatic continuation without changing active/paused persistence.
