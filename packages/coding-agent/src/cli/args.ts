	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--help" || arg === "-h") {
			...
			if (i + 1 < args.length) {
				...
				result.diagnostics.push({ type: "error", message: "--worktree requires a value" });
			}
			...
			result.excludeTools = args[++i]
				...
				.map((s) => s.trim())
				.filter((name) => name.length > 0);
		} else if (arg === "--permission-prompt-tool") {
			if (i + 1 < args.length) {
				result.permissionPromptTool = args[++i];
			} else {
	...
	const extensionFlagsText =
		extensionFlags && extensionFlags.length > 0
			? `\n${chalk.bold("Extension CLI Flags:")}\n${extensionFlags
					...
					.join("\n")}\n`
			: "";
	console.log(`${chalk.bold(APP_NAME)} - AI coding assistant with file, command, and structural code tools
...
${chalk.bold("Commands:")}
  ...
  ${APP_NAME} debug attach <session-id> Attach to an enabled live-process debug REPL
  ${APP_NAME} sessions archive           Archive sessions older than five days
  ${APP_NAME} sessions truncate-tool-output  Truncate oversized persisted tool results
...
${chalk.bold("Options:")}
  ...
  --permission-prompt-tool <mcp__server__tool>
                                 Delegate tool approval decisions to an MCP tool
