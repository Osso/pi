...
- [x] `manage_goal set` sends current and proposed objectives as claims. Reviews also receive the latest genuine same-session user input as `userRequest`, including before the first goal exists; extension input cannot replace it, another session cannot inherit it, and absent raw input is never reconstructed from assistant text. Preserve requirements grounded in explicit user instructions or applicable persistent project contracts, remove unsupported plan/task-ID constraints, collapse exact repeated scope before persistence, and leave state unchanged on failed or stale review.
...
- [x] Interactive Escape cancels an active goal review through global input even while the caller is idle (the editor keeps focus while the waiting status shows): pending or claimed durable requests become `cancelled`, client retries stop, resident evaluation aborts, the loader clears, the goal and unconsumed evidence remain active, and late decisions cannot apply.
...
- [x] Autonomous continuation has no numeric turn cap; Supervisor review preserves agent autonomy and the full objective, using exactly `Continue working toward the active goal.` when the agent can determine its own next step and emitting a visible `supervisor` corrective follow-up only for an evidence-backed exception such as unhandled pagination, an omitted required element, repeated failed or circular work, lost objective scope, or missing completion proof, naming only the exception and smallest corrective action; corrective follow-ups retain explicit Supervisor provenance for model context, while the renderer shows one `[Supervisor]` header and a plain instruction body without exposing the XML wrapper; `complete` closes the goal; `wait` or idle-review `error` appends durable status, calls `wait_agent` when agents are active, and re-runs Supervisor review after agent wake or fifteen minutes; recheckable external conditions use the same scheduled wait path and countdown; wake evidence includes structured details and visible tool-result content so coordination instructions survive the wait handoff; only `pause` for required user action or input stops automatic continuation without changing active/paused persistence.
- [x] Every automatic Supervisor wait persists exactly one absolute fifteen-minute `reviewAt` deadline and renders `Next review in M:SS` from that deadline, including active-agent `wait_agent` waits; repainting never appends countdown entries or triggers duplicate review work, and expiry shows `Review due…`. Countdown repaint requests update only the rendered Supervisor status entry; unsafe or offscreen repaint attempts are skipped without scheduling a generic root render. Agent completion may trigger review early; the deadline cancels `wait_agent` and triggers review when completion has not arrived; the first wake path wins, cancels the other path, and schedules no duplicate review. A `wait_agent` failure retains the original deadline for timed review. Continuation cancellation stops the wait and redraw timer, and session restore resumes redraw-only countdown state without recreating review work.
...
- `packages/coding-agent/extensions/goal/src/goal-scheduling.ts` — preserves decisions across transient pending input, waits for active agents, and schedules fifteen-minute Supervisor re-review from the persisted deadline.
...
- `packages/coding-agent/extensions/goal/src/rendering.ts` — persists Supervisor status deadlines, binds rendered status entries to scoped redraw callbacks, and renders dynamic countdown text without owning timers.
- `packages/coding-agent/src/modes/interactive/components/render-region-container.ts` — tracks rendered custom-entry row bounds and attempts entry-only redraws without generic fallback.
- `packages/tui/src/tui.ts` — exposes the no-fallback render-region attempt used for skipped unsafe countdown ticks.
...
- [x] Show the exact fifteen-minute Supervisor re-review deadline as a live, persisted, cancellation-safe countdown without creating duplicate review work or transcript entries.
