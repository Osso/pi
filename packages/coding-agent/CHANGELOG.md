...
- Fixed steering mutations reserving SQLite's writer slot during agent, ownership, transition, and listener validation; exact snapshot revalidation now keeps the agent update, message-counter allocation, and canonical mailbox persistence atomic.
...
- Fixed detached-job finalization reserving SQLite's writer slot during ownership/path lookup, replay, lifecycle, and descendant validation; read-only preflight now resolves one exact owner path, while commit-time uniqueness and recursive descendant CAS atomically persist the terminal agent, outbox row, and optional detached transport notification.
...
- Fixed direct dead-runtime recovery holding the shared `control.sqlite` writer lock during supervisor, agent, owner, descendant, and process-liveness validation; recovery now preflights those checks read-only, revalidates persisted supervisor/listener authority, exact owner and agent snapshots, and recursive descendant absence with commit-time CAS, then atomically persists ownership release, terminal state, the outbox row, and any detached transport notification under the short lock.
