...
| Change working directory tool (`change_working_directory`) | [`change-working-directory-tool.md`](change-working-directory-tool.md) | **BUILD** (done) | Built-in tool for persistent cwd changes by path or another session's recorded cwd without session replacement. |
