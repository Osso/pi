...
- `spawn_agent` now requires a `context` value: `fresh` starts the child with only its assignment, while `inherit` forks the persisted parent transcript before appending the assignment; callers should use `inherit` for required prior decisions/research and `fresh` for isolated work, review, verification, or falsification. Background jobs and restart recovery use `fresh`, and attached-session reuse is separate.
