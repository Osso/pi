...
- Added a distinct Supervisor `wait` goal decision that emits a durable status message, waits through `wait_agents` when agents are active, and otherwise re-runs Supervisor review after five minutes without duplicate continuation.
