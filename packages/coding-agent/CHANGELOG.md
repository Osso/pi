...
- Fixed compacted session restart loading the summarized JSONL prefix into memory; resume now reverse-loads only the active slice, retains the complete transcript on disk, and restores current cwd/model/thinking from control SQLite without adding JSONL setting entries.
