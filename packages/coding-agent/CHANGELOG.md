...
- Added a distinct Supervisor `wait` goal decision that emits durable status, runs cancellable background `wait_agents` when agents are active, and otherwise re-runs Supervisor review after five minutes; transient pending-message suppression preserves reviewed decisions, while input, goal changes, new turns, and shutdown cancel stale scheduling.
