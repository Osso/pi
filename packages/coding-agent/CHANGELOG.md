...
- Added runtime-only `/fast` mode for `openai-codex` and `openai-codex-gc`: bare `/fast` and `/fast on` select `priority`, `/fast ultra` selects `ultrafast`, and `/fast off` disables it; one main-thread authority shares the selected tier dynamically with spawned and attached child runtimes, while child `/fast` commands cannot mutate it and main session start, restart, or resume resets it without persistence.
