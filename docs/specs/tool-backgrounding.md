...
- [x] Before detachment, a durable Pyrun wrapper `.runner-error` sidecar promptly terminalizes the foreground tool call as failed only after already-present artifact bytes are drained so canonical success/error records take precedence; nonempty sidecar text is surfaced, empty sidecars produce an explicit diagnostic, only the exact still-live runner process identity is terminated, and child-agent tool activity reaches normal terminal settlement.
