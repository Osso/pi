...
- Added a distinct Supervisor `wait` goal decision that emits durable status, runs cancellable background `wait_agents` when agents are active, and otherwise re-runs Supervisor review after five minutes; idle-review errors use the same self-waking path, transient pending-message suppression retries until input drains while preserving decisions and wake evidence, asynchronous reviews recheck goal identity, agent-discovery failures fall back to timed review, and input, goal changes, new turns, or shutdown cancel stale work.
...
- Fixed detached `running` and `cancelling` jobs remaining wedged after their exact runner died: startup globally reconciles rows whose dead runner identity matches the persisted worker handle, has no active descendant or replacement owner, and has no terminal outbox, regardless of parent-session liveness, without reparenting or duplicate terminalization.
