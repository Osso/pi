...
- Fixed runtime-owned child creation acquiring the shared `control.sqlite` writer lock for pure payload, ID, lifecycle, parent-ID validation, and serialization; those checks now complete before locking while parent activity, uniqueness, and agent/ownership persistence remain atomic.
- Fixed attached-agent creation acquiring the shared `control.sqlite` writer lock for pure payload, ID, origin, lifecycle, revision validation, and serialization; those checks now complete before locking while uniqueness, parent permission, and insertion remain atomic.
- Fixed failed-child creation acquiring the shared `control.sqlite` writer lock for pure payload, lifecycle, revision, parent-ID validation, and serialization; those checks now complete before locking while parent activity, uniqueness, and agent/terminal-outbox persistence remain atomic.
