...
- [x] Closing a tall temporary editor or selector with Escape resets an upward-moving viewport so the restored editor and footer remain bottom-anchored, repainting only the visible window instead of clearing and replaying the history; frames whose visible window contains image lines keep the full repaint (`packages/tui/test/root-compositor.test.ts`: "restores bottom anchoring after a tall editor replacement closes with Escape").
