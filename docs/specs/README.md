...
| Current session history search (`search_current_session_history`) | [`current-session-history-search.md`](current-session-history-search.md) | **BUILD** (done) | Active-branch search across full persisted history, including entries omitted by compaction. |
