...
- Fixed lifecycle mutations reserving SQLite's writer slot during ownership/lifecycle validation and detached-cancellation payload preparation; validation now runs before the transaction, while exact snapshot revalidation keeps lifecycle and cancellation persistence atomic.
- Fixed steering mutations reserving SQLite's writer slot during agent, ownership, transition, listener, and mailbox-payload preparation; validation and agent serialization now run read-only, while message-counter allocation remains inside the atomic transaction so rollback preserves numbering and agent/mailbox persistence.
- Fixed steering delivery reserving SQLite's writer slot during agent, ownership, transition, and canonical message validation; exact agent and message snapshot revalidation now keeps delivery's coupled row updates atomic.
- Fixed terminal mutations reserving SQLite's writer slot during ownership, replay, transition, and descendant validation; read-only preflight now precedes the transaction, while commit-time recursive descendant CAS preserves exact ownership and atomically persists the terminal agent and outbox row.
- Fixed detached-job finalization reserving SQLite's writer slot during ownership/path lookup, replay, lifecycle, and descendant validation; read-only preflight now resolves one exact owner path and prepares detached transport JSON/routing, while commit-time uniqueness and recursive descendant CAS atomically persist the terminal agent, outbox row, and prepared transport insert.
