...
- Transcript and mailbox/contact activity metadata now merge transactionally into the latest persisted agent snapshot without rewriting lifecycle/revision from stale store state; activity updates no longer advance the lifecycle revision token, and restore no longer persists runtime worker-handle cleanup.
