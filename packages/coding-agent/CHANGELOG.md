...
- Added optional `willRetry` to extension `agent_end` events so extensions can distinguish retryable errors from exhausted retries.
...
- Fixed `/goal` continuation after an assistant error: it now leaves the active goal intact without queuing a follow-up or showing the empty-response warning, so retry/session error handling owns recovery; skipped-status output is deferred while `agent_end.willRetry` is true and emitted once retries are exhausted.
