...
- [x] The `manage_goal` completion action requests resident Supervisor review for active goals, including paused goals; `complete` marks the goal complete, `continue` keeps it active with concrete next-step instructions, and `pause` pauses it when progress must wait.
