...
- Fixed OpenAI Codex OAuth credentials to preserve the user email from the login id token when available.
...
- Increased the OpenAI Codex Responses SSE response-header timeout to 3 minutes to reduce false-positive stalls while retaining the bounded wait introduced for zero-event hangs ([#4945](https://github.com/earendil-works/pi/issues/4945)).
