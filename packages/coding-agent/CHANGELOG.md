...
- Split agent notification waiting from notification consumption so internal callers can wait for terminal or coordination input without consuming completion delivery; `wait_agents({})` preserves its existing behavior by composing both operations.
