...
- Changed `/archive`, picker, and bulk-archived session storage to compress transcripts as `.jsonl.zst`; resuming restores plain `.jsonl` storage and active metadata. Resident Supervisor and Architect transcripts remain metadata-archived plain JSONL because their live runtimes append to them.
