...
describe("openai-codex streaming", () => {
	...
	it("reports SSE HTTP retries through onRetry", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				codexCalls += 1;
				if (codexCalls === 1) {
					return new Response("server error", { status: 500, headers: { "retry-after-ms": "0" } });
				}
		...
		expect(retries).toEqual([{ attempt: 1, maxAttempts: 2, delayMs: 0, reason: "HTTP 500" }]);
	});
	...
	it.each([
		["max", "max"],
		["ultra", "max"],
	] as const)("serializes GPT-5.6 Sol %s as provider effort %s", async (logicalEffort, providerEffort) => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
		const sse = buildSSEPayload({ status: "completed" });
		...
		await streamSimpleOpenAICodexResponses(OPENAI_CODEX_MODELS["gpt-5.6-sol"], context, {
			apiKey: token,
			reasoning: logicalEffort,
			transport: "sse",
		}).result();

		expect(requestedReasoning).toEqual({ effort: providerEffort, summary: "auto" });
	});
