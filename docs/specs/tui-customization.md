...
- [x] While any component other than the message editor has focus (built-in selectors, extension dialogs, inputs, overlays), a key bound to both `app.interrupt` and `tui.select.cancel` reaches that component and does not interrupt the active turn; a key bound only to `app.interrupt` still interrupts, and Escape in the editor still interrupts. No per-dialog exemption exists (`interactive-mode-status.test.ts`: "escape closes any focused dialog without interrupting, while escape in the editor still interrupts").
- [x] Key-release events (kitty keyboard protocol) never trigger `app.interrupt` or agent-slot actions, so releasing Escape after a dialog closes on press does not interrupt the turn (`interactive-mode-status.test.ts`: "kitty escape press and release close the settings selector without interrupting a streaming turn").
...
- [x] Closing a tall temporary editor or selector with Escape resets an upward-moving viewport so the restored editor and footer remain bottom-anchored, repainting only the visible window instead of clearing and replaying the history; frames whose visible window contains image lines keep the full repaint. Accepted tradeoff: lines the tall selector had pushed into terminal scrollback remain there and reappear in the repainted window, so scrolling up can show up to one selector height of duplicated lines; replaying the whole history on every selector close was judged worse (`packages/tui/test/root-compositor.test.ts`: "restores bottom anchoring after a tall editor replacement closes with Escape").
...
- [x] Registered flow render regions allow a flow component to update or grow without rerendering earlier flow components. Cached downstream flow and bottom rows, terminal scrollback, registered regions, fixed cells, and the hardware cursor remain coherent; unsafe or shrinking updates fall back to normal root rendering (`render-region.test.ts`: "grows a flow tail without rendering surrounding components", "appends and routes a new descendant from an empty flow tail", "grows a visible flow tail while preserving terminal scrollback", "preserves a bottom hardware cursor while a flow tail grows", "shifts a fixed bottom cell after flow-tail growth", "shifts a registered bottom region after flow-tail growth", and "falls back to a normal root render when a flow tail shrinks").
...
- [x] Selecting a plaintext compaction summary in `/tree` loads its complete persisted text into the normal message editor, including at the current leaf. Selecting an OpenAI-native compaction first materializes a complete plaintext summary through one current model call whose provider/API exactly matches the saved `openai.responses.input` checkpoint; no provider/model fallback is used. Materialization has no persistence effect, and Escape or failure leaves the entry unchanged. Submitting non-command text updates only the summary; for native entries, save atomically removes `providerNative` and native compaction details, preserves identity, tree position, leaf, and token/duration metadata, then rebuilds active context and transcript from the edited plaintext (`interactive-mode-compaction-edit.test.ts`, `interactive-mode-compaction-rendering.test.ts`, `agent-session-compaction-summary-edit.test.ts`, `update-compaction-summary.test.ts`).
- [x] `hideToolOutput: true` in settings.json (or "Hide tool output" in `/settings`, applied live to existing cards) hides tool result text and images in the transcript for built-in, extension, and definition-less tools, regardless of Ctrl+O expansion; call lines and elapsed timers stay visible (a quiet running bash timer keeps ticking and stops on completion) and the model still receives full output (`tool-execution-component.test.ts`: "hideOutput keeps the call and drops result output until disabled", "hideOutput drops result output for tools without a definition", "hideOutput keeps a quiet running bash elapsed timer ticking and stops it on completion"; `settings-manager.test.ts`: "hideToolOutput").
...
- [x] Same-height coding-agent status-loader text and elapsed updates use the registered bottom status region; dynamic height changes and unregistered loaders retain normal rendering. Assistant message start and visible text/thinking stream updates use the registered transcript-tail flow region, while message completion consolidates the tail into chat and performs a normal render. User messages, tool layouts, history rebuilds, and other transcript mutations retain normal rendering (`interactive-root-compositor.test.ts`, `interactive-mode-streaming-render-throttle.test.ts`, `loader.test.ts`, `interactive-mode-idle-notification.test.ts`, `tool-execution-component.test.ts`).
- [x] Invisible assistant stream updates do not schedule renders: hidden thinking deltas, partial tool-call/protocol deltas, and hidden main-session message updates while viewing a child are ignored. Visible text/thinking updates schedule transcript-tail region renders, while final message completion uses normal rendering (`interactive-mode-streaming-render-throttle.test.ts`).
...
- `packages/coding-agent/src/modes/interactive/interactive-root-compositor.ts` and `interactive-mode.ts` — preserve flow order, anchor the interactive bottom zone, register status and transcript-tail render regions, route visible assistant streaming through the tail, consolidate completed messages into chat, track prompt activity sources, and create static coding-agent status loaders.
...
- `packages/tui/src/tui.ts` and `root-compositor.ts` — record render-region generations and flow/bottom layout bounds, route descendant update requests, update or grow bounded flow regions, preserve cached downstream rows and terminal scrollback, and reject unsafe partial updates.
...
- `packages/coding-agent/src/modes/interactive/interactive-mode.ts` (`registerGlobalInterruptInputHandler`) — global interrupt listener: ignores key releases and yields `tui.select.cancel` to any focused non-editor component; `TUI.getFocusedComponent()` in `packages/tui/src/tui.ts` exposes the focus it checks.
...
- `packages/coding-agent/test/interactive-root-compositor.test.ts` — actual interactive flow/bottom ordering, transcript-tail layout registration and growth without unrelated root-entry renders, editor repositioning, and nested status Loader updates without unrelated root-entry renders.
- `packages/coding-agent/test/interactive-mode-status.test.ts` — dialog-owned cancel keys and key-release filtering for the global interrupt listener.
- `packages/tui/test/root-compositor.test.ts` — bottom anchoring after normal layout changes and after a tall temporary editor replacement closes with Escape.
- `packages/tui/test/render-region.test.ts` — "renders a same-height descendant update only inside its registered region", "updates the visible rows of a flow tail that starts above the viewport", "synchronizes the previous frame after a partial region update", and "refreshes descendant routing after a same-height subtree replacement" cover bounded same-height updates and cache/routing coherence; "grows a flow tail without rendering surrounding components", "appends and routes a new descendant from an empty flow tail", "grows a visible flow tail while preserving terminal scrollback", "preserves a bottom hardware cursor while a flow tail grows", "shifts a fixed bottom cell after flow-tail growth", and "shifts a registered bottom region after flow-tail growth" cover flow append/growth and downstream state; "falls back to a normal root render when a flow tail shrinks", "falls back to a normal root render when the region height changes", "uses normal rendering when a render, resize, or visible overlay makes placement unsafe", "rejects a placement not refreshed by the latest root render", and "falls back to normal rendering for an unregistered component" cover fallback behavior. `render-region-safety.test.ts` and `loader.test.ts` cover additional safety and unregistered Loader behavior.
