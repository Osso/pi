...
Explicit `manage_goal complete` requests `goal_completion_review` before changing goal state. `continue` leaves the goal running and queues a concrete next action. `complete` marks it complete. `pause` leaves it active without queueing another continuation when progress requires waiting, remaining idle, or external input.

Idle review remains inside the existing `agent_end` handler after its pending-message, abort, error-stop, and empty-response retry handling. Pending interactive input takes precedence over abort handling; aborted turns do not change persisted goal state. A non-error empty assistant response schedules one continuation after a 1-second bounded delay only if the same goal remains active, the session is idle, and no messages are pending; session shutdown cancels that timer. A Supervisor `pause` decision leaves the goal active without submitting another continuation, preventing idle-review loops. `agent_end` already means the tool loop reached a terminal response with no further tool calls; no additional tool-call check exists or is needed. The previous unconditional continuation message is replaced by `goal_idle_review`.
