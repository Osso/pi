...
- [x] Prove `wait_agent` remains blocked when `steer_agent` input was already pending before the wait started (no stale wake), the steering reaches the child's next LLM request, and the wait returns only after that full child turn completes and terminalizes.
