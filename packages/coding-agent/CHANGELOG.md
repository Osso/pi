...
- Fixed durable foreground Pyrun wrapper failures leaving child agents indefinitely on `Waiting for tool: pyrun_eval`; pre-detach `.runner-error` sidecars now fail the tool call, preserve nonempty diagnostics, report empty sidecars explicitly, and terminate the foreground process tree.
