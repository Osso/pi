...
On September 13, 2026, regression `f8cc545d7` reproduced the same stale-context stack after a production child started a loop, completed, and was disposed without `session_shutdown`. Fix `74fbe26ce` makes spawned and attached child teardown await that event before disposal; `1243a6a75` repaired its test barrier. The focused suite passed 244 tests (`/tmp/pi-loop-targeted.log`), the barrier test passed 1/1 (`/tmp/pi-loop-shutdown-test.log`), and `deploy.sh` completed (`/tmp/pi-loop-deploy-final.log`). Installed SHA-256: `1f1fa8997e4dd90c067da67d4db4146f1d6ec2d2f30484b463083ad146a9d592`. Only eligible live main session `01a09920-be0c-7672-870a-fa5dc084c355` (PID 3945823) re-execed the installed launcher and returned `running`/`ok` with that executable hash (`/tmp/pi-loop-rollout-after.json`). The WoW session was not live and was not restarted. Independent verification remains pending. This reproduction does not identify the original incident's exact trigger.
