...
it.skipIf(process.platform !== "linux")("reaps active memory enrichment before process restart", async () => {
		...
		await withHeadlessPi(
			async (agent) => {
				...
				try {
					...
					if (!(error instanceof Error) || !error.message.includes("Timed out waiting for LLM request")) throw error;
