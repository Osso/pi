...
- Added duplicate-turn loop detection: after consecutive identical text-only assistant turns, Pi injects a non-persisted instruction to call the always-active `end_turn` control tool with a concise reason; changed assistant content, tool execution, or new user input resets detection. Ordinary tool filtering remains unchanged.
...
- Fixed compacted session restart loading the summarized JSONL prefix into memory; current-version resume now reverse-scans the active parent chain through `firstKeptEntryId`, retains only the active slice in memory, preserves the untrimmed transcript and immutable initial-cwd header on disk, restores current cwd/model/thinking from control SQLite, uses configured defaults without backfill when model/thinking metadata is absent, and rejects corrupt active JSONL while ignoring one incomplete trailing line.
