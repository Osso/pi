...
- [x] Reopening a source session whose active transcript ends with a `resume_session` tool call leaves the source session active without appending a `toolResult` for that call.
