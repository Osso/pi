...
`queue_update` emits the full pending steering and follow-up queues whenever they change. `bash_messages_committed` emits after idle or deferred bash messages are appended to session state. `compaction_start` and `compaction_end` cover both manual and automatic compaction.
...
type AgentEvent =
  ...
  | { type: "tool_execution_end"; toolCallId: string; toolName: string; result: any; isError: boolean; startedAt: number; finishedAt: number };
```
