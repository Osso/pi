...
- Restored Linux deployment ownership for the installed `pi-architect.service` systemd user service while keeping the `ask_architect` built-in tool disabled; default deployment now uses Pi-managed lazy Supervisor startup and removes only the Supervisor user unit, while Darwin skips systemd configuration and `PI_DEPLOY_CONFIGURE_RESIDENT_SERVICES=1` remains the Supervisor systemd opt-in.
