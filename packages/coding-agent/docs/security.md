...
Declining trust skips protected resources. `AGENTS.md`, `AGENTS.local.md`, `CLAUDE.md`, `CLAUDE.local.md`, and project `docs/local/memory.md` context files are loaded regardless of project trust unless context loading is disabled. In each searched global or cwd-ancestor directory, Pi loads every AGENTS-family candidate that reads successfully when at least one AGENTS-family candidate loads successfully there; CLAUDE-family candidates are loaded only when no AGENTS-family candidate loads successfully there. Project memory is read only from cwd ancestors, never from the global agent directory. Before trust is resolved, pi only loads context files, user/global extensions, and CLI `-e` extensions. User/global and CLI extensions can handle the `project_trust` event; the first extension that returns a yes/no decision owns the decision.
