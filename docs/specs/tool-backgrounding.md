...
- `packages/coding-agent/extensions/pyrun/src/index.ts` — registers Pyrun evaluations as detachable and tracks detached evaluations. Pyrun command guidance treats both `run.*` and default `cli.*.run()` as forwarding, exit-code-only execution; `.capture().run()` explicitly returns a `CommandResult`.
