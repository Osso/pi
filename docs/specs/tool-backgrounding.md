...
- [x] Before detachment, a durable Pyrun wrapper `.runner-error` sidecar settles the foreground tool call as failed after any canonical terminal record is considered; empty sidecars produce an explicit diagnostic, the foreground process tree is terminated, and child-agent tool activity can reach normal terminal settlement.
...
- `packages/coding-agent/test/pyrun-extension.test.ts` — detached Pyrun script/output persistence, foreground wrapper-sidecar settlement, completion/failure regressions, elapsed `durationMs`, and duration-bearing lifecycle notifications.
