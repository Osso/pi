...
function utf8Prefix(text: string, maxBytes: number): string {
	...
	while (end > 0) {
		...
		if (utf8Bytes(prefix) === end) return prefix;
		end -= 1;
	}
	return "";
}
...
export function truncateToolResultForPersistence<T extends SessionMessageEntry>(entry: T): T {
	...
	const content = capToolResultContent(entry.message.content);
	if (content === entry.message.content) return entry;
	return { ...entry, message: { ...entry.message, content } } as T;
}
...
function readSessionFileForMigration(filePath: string): SessionFileParseResult {
	...
	return { kind: "session", lines, entries };
}
...
function writeSessionFileAtomically(filePath: string, content: string, now: Date): string {
...
function rewriteSessionFile(
	...
	return writeSessionFileAtomically(filePath, content, now);
}
