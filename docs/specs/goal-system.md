...
- [x] Extension `agent_end` events expose optional `willRetry`; `true` means the current request will be retried before goal error handling reports a terminal failure.
...
- [x] If the last assistant message has `stopReason: "error"`, goal continuation neither queues a follow-up nor emits the empty-response warning; when `event.willRetry` is `true`, retry/session error handling owns recovery and no skipped-status entry is emitted, while the exhausted retry emits one durable error status and leaves the active goal intact.
...
- `packages/coding-agent/src/core/extensions/types.ts` — exposes optional `AgentEndEvent.willRetry` to extensions.
