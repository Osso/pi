...
- Added per-session `/fast` mode for `openai-codex` and `openai-codex-gc`: bare `/fast` toggles the current mode, `/fast on` selects `priority`, `/fast ultra` selects `ultrafast`, and `/fast off` disables it; one main-thread authority shares the selected tier dynamically with spawned and attached child runtimes, while child `/fast` commands cannot mutate it and accepted changes persist outside LLM context across `/restart` and later reopening of the same session.
