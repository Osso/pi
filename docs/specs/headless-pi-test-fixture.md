...
- [x] Prove `wait_agents` remains blocked when `steer_agent` input was already pending before the wait started (no stale wake), the steering reaches the child's next LLM request, and the wait returns only after that full child turn completes and terminalizes.
...
- [x] Prove real-process replacement restores only the active compacted JSONL slice in memory, relocates the session without dropping its summarized prefix, restores again from the relocated file with current cwd/model/thinking from `session_metadata`, and does not append model/thinking JSONL entries (`regressions/session-active-slice-restart.test.ts`).
- [x] Prove reopening accepts an already-relocated retained slice without weakening uncompacted broken-chain rejection (`active-slice-load.test.ts`).
