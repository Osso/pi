...
- [x] Streaming output accumulation remains capped to the most recent 300 lines and 1 MiB (1,048,576 UTF-8 bytes), preserving recent output even when a single line is very large.
...
- `packages/coding-agent/extensions/pyrun/src/eval-tool.ts` — requests console streaming and converts console events into cumulative tool updates capped by the most recent 300 lines and 1 MiB of UTF-8 text.
