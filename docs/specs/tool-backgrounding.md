...
- [x] Unfinished foreground Bash recovery permits one replacement execution only when the matching persisted job is terminal `failed` with `lost_runtime`; ordinary failed and cancelling jobs are not rerun (`packages/coding-agent/test/suite/headless-pi.test.ts`).
