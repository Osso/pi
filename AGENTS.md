...
- Documentation-update sub-agents default to fresh context. Select inherit only when conversation history is necessary for the task.
...
- Readability gate for canonical first-party maintained extensions: when a change affects any listed directory under `packages/coding-agent/extensions/`, run the `ts-readability` audit on every TypeScript/JavaScript/QML source file in each affected listed extension, not only changed files. New or worsened violations block completion, including the 750-line file threshold. Unchanged baseline violations outside the scoped diff do not block completion when remediating them would materially increase upstream merge conflicts; record them separately. Exclude examples, tests, generated/dist, vendored, and private/external extensions. The post-removal canonical directory list is:
...
- Commit verified fixes immediately, one logical change at a time, unless the user explicitly asks not to commit.
...
- After deployment, enumerate eligible live Pi sessions once, recording launcher, PID/start identity, and executable or source revision. Force-restart every runtime still using the superseded artifact without additional confirmation; do not restart current runtimes.
- Preserve launchers: installed `pi` sessions re-exec the deployed binary, while `pi-dev` sessions re-exec from canonical source. Never migrate `pi-dev` to installed `pi`.
- After replacements, run one bounded final identity and health inventory. Do not poll repeatedly.
