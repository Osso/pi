...
describe("buildSystemPrompt", () => {
	...
	describe("command backgrounding guidelines", () => {
		test("warns that supported long-running command tools auto-background after 5 minutes when wait_agents is available", () => {
			const prompt = buildSystemPrompt({
			...
			expect(prompt).toContain(
				"Supported long-running command tools such as bash and Pyrun are automatically backgrounded after 5 minutes; use wait_agents to wait for any agent completion, then inspect reported background job details or mailbox/log artifacts instead of assuming the command stopped.",
			);
		});
