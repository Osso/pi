...
async function loginOpenAICodexOAuthProvider(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
...
async function refreshOpenAICodexOAuthProviderToken(credentials: OAuthCredentials): Promise<OAuthCredentials> {
	return refreshOpenAICodexToken(credentials.refresh);
}

function getOpenAICodexOAuthProviderApiKey(credentials: OAuthCredentials): string {
	return credentials.access;
}

function createOpenAICodexOAuthProvider(id: string, name: string): OAuthProviderInterface {
	return {
		id,
		name,
		usesCallbackServer: true,
		login: loginOpenAICodexOAuthProvider,
		refreshToken: refreshOpenAICodexOAuthProviderToken,
		getApiKey: getOpenAICodexOAuthProviderApiKey,
	};
}

export const openaiCodexOAuthProvider: OAuthProviderInterface = createOpenAICodexOAuthProvider(
	"openai-codex",
	"ChatGPT Plus/Pro (Codex Subscription)",
);
export const openaiCodexGcOAuthProvider: OAuthProviderInterface = createOpenAICodexOAuthProvider(
	"openai-codex-gc",
