...
- [x] Before detachment, a durable Pyrun wrapper `.runner-error` sidecar promptly terminalizes the foreground tool call as failed after canonical terminal records are checked; canonical success/error records take precedence, nonempty sidecar text is surfaced, empty sidecars produce an explicit diagnostic, and sidecar settlement terminates the foreground process tree so child-agent tool activity reaches normal terminal settlement.
