...
Every automatic Supervisor wait calculates one absolute five-minute `reviewAt` deadline and persists that same value in its durable `supervisor-status` entry, including active-agent `wait_agent` waits. Interactive rendering derives `Next review in M:SS` from the deadline on each paint and switches to `Review due…` at expiry. A separate deadline-aligned refresher only requests UI redraws; it never appends entries, invokes Supervisor review, or changes the five-minute schedule.

Active-agent completion may wake review before the deadline. If the deadline arrives first, it cancels `wait_agent` and starts review; the first path wins, cancels the losing path, and schedules no duplicate review. A `wait_agent` failure retains the original deadline for timed review. Input, new turns, goal changes, review start, session replacement, and shutdown clear redraw timers beside the existing review schedules. Resume restores redraw-only refresh from the newest future deadline but does not recreate review work.
